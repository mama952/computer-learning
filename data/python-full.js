  ...
  'lessons': [
    ...
    },
    {
      ...
      'blocks': [
        ...
        {
          'type': "h3",
          'text': "遇到多变量：先给每个变量画一个「盒子」"
        },
        {
          'type': "p",
          'text': "单变量大家都会，一到多个变量同时出现就晕。破解方法只有一个：<strong>任何时刻，把每一个变量都想成一个贴了名字的盒子，盒子里只放一个数。</strong>再多、再绕的题，只要握住下面两条铁律，都能一眼看穿。"
        },
        {
          'type': "p",
          'text': "<strong>铁律一（盒子）：</strong>每个变量 = 一个贴标签的盒子，里面只放一个数。每次给这个变量<code>=</code>赋值，就是把盒子里的旧数<strong>换掉</strong>，换成 <code>=</code> 右边算出来的新数。<br/><strong>铁律二（抄写）：</strong><code>y = x</code> 只做一件事——把 x 盒子<strong>当前</strong>那个数「抄一份」放进 y 盒子。抄完这一瞬间开始，x、y 就是<strong>两个互相独立的盒子</strong>，以后改谁都不影响另一个（对数字这种值类型）。"
        },
        {
          'type': "p",
          'text': "<strong>万能节奏：一次只看一行，只盯一个变量。</strong>遇到多变量永远按这三步：① 看这一行 <code>=</code> 右边是什么式子；② 式子里的变量把「现在这一刻」的值代进去，算出一个数；③ 把这个数盖到左边那个变量的盒子里，旧的作废。然后才看下一行。"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "主例子：x=1; y=x; y=3; print(y) —— 逐行看盒子",
          'code': "# 第1行：造一个叫 x 的盒子，放入 1\nx = 1        # 此刻：x=1，y 还不存在\n\n# 第2行：把 x 盒子里【现在】的 1，抄一份给 y\n#       也就是新建 y 盒子，放进 1\n#       此刻 x=1、y=1，两个盒子各有一份 1\ny = x\n\n# 第3行：只动 y 的盒子，把里面的 1 换成 3\n#       x 的盒子碰都没碰，还是 1\ny = 3        # 此刻：x=1，y=3\n\n# 第4行：读 y 盒子，里面是 3 → 输出 3\nprint(y)     # 输出：3\n\n# 顺便读读 x\nprint(x)     # 输出：1"
        },
        {
          'type': "table",
          'head': ["执行的代码行", "这一行干了什么", "x 盒子", "y 盒子"],
          'rows': [
            ["x = 1", "创建 x 盒子，放入 1", "1", "（还不存在）"],
            ["y = x", "把 x 当前的值 1 抄一份，新建 y 盒子放入", "1", "1"],
            ["y = 3", "只把 y 盒子里的 1 换成 3，x 不动", "1", "3"],
            ["print(y)", "读 y 盒子的值", "1", "3 → 打印 3"],
            ["print(x)", "读 x 盒子的值", "1 → 打印 1", "3"]
          ]
        },
        {
          'type': "p",
          'text': "请仔细观察上面的表：<strong>每到新的一行，只有被 <code>=</code> 左边点名的那一个变量会变，右边的变量只是被看了一眼、用它的值算了一下，自己并不改变。</strong>x 从头到尾没有第二行再被赋值，所以它永远是 1。"
        },
        {
          'type': "h3",
          'text': "变式题库：把每种绕法都拆开，逐个变量、逐行讲"
        },
        {
          'type': "p",
          'text': "下面 6 个变式，几乎覆盖了考试和面试里所有「多变量」的坑。建议这样学：<strong>先遮住解析，自己在心里用『盒子三步法』推一遍答案，再对照代码和表格看每一步。</strong>推错的也没关系，看它是错在哪一步，比直接看答案记得牢。"
        },
        {
          'type': "h4",
          'text': "变式①：改了 x，y 会跟着变吗？"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式①代码",
          'code': "x = 1\ny = x        # 把 x【现在】的 1 抄给 y → y=1\nx = 2        # 这里改的是 x，y 是另一个盒子\nprint(y)     # 你以为会变 2？错——输出 1\nprint(x)     # 输出 2"
        },
        {
          'type': "p",
          'text': "<strong>变式①逐行看盒子：</strong>很多人误以为 <code>y = x</code> 之后 x、y 就『绑死』、x 变 y 也变。<strong>错的。</strong>它们的血缘只存在于 <code>y = x</code> 执行的那一瞬间：y 拿走的是当时的 1，抄完就断。之后 x=2 只改 x 这个盒子，跟 y 无关。"
        },
        {
          'type': "table",
          'head': ["代码行", "x 盒子", "y 盒子", "说明"],
          'rows': [
            ["x = 1", "→ 1", "（还不存在）", "x 放入 1"],
            ["y = x", "1", "→ 1", "把此刻的 1 抄给 y"],
            ["x = 2", "1 → 2", "1（不动）", "只改 x，y 独立"],
            ["print(y)", "2", "1 → 打印 1", "y 仍是 1"]
          ]
        },
        {
          'type': "h4",
          'text': "变式②：v = v + 1 —— 同一个变量自己更新自己"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式②代码",
          'code': "v = 2\nv = v + 1    # 先算右边 2+1=3，再存回 v（v 变 3）\nv = v * 3    # 先算右边 3*3=9，再存回 v（v 变 9）\nprint(v)     # 输出 9"
        },
        {
          'type': "p",
          'text': "<strong>变式②逐行看盒子：</strong><code>v = v + 1</code> 是最容易被绕的一句。记住一句话：<strong>赋值号 = 左边是目标盒子，右边是算式，永远先算完右边、再把结果盖进左边。</strong>算右边时用的是 v『现在这一刻』的旧值，算完这个旧值就作废，被新结果覆盖。"
        },
        {
          'type': "table",
          'head': ["代码行", "v 的旧值", "先算 = 右边", "v 的新值"],
          'rows': [
            ["v = 2", "（还未创建）", "—", "2"],
            ["v = v + 1", "2", "2 + 1 = 3", "3"],
            ["v = v * 3", "3", "3 * 3 = 9", "9"],
        ...
        {
          'type': "h4",
          'text': "变式③：三个变量大联动（读哪个、改哪个要分清）"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式③代码",
          'code': "a = 5\nb = a        # b 抄 a 的 5 → b=5\na = 10       # 改 a，b 不受影响 → b 还是 5\nc = a + b    # 算右边 a+b=10+5=15，放进 c\nprint(a, b, c)   # 输出：10 5 15"
        },
        {
          'type': "p",
          'text': "<strong>变式③要看的关键：</strong>三个盒子并存时最容易慌。回到铁律：<strong>每一行只改 <code>=</code> 左边那一个，右边的只是被拿去算数，自己不变。</strong>a 最后被赋 10，b 从没被改（一直是 5），c 是 10+5=15。"
        },
        {
          'type': "table",
          'head': ["代码行", "a 盒子", "b 盒子", "c 盒子"],
          'rows': [
            ["a = 5", "→ 5", "（还不存在）", "（还不存在）"],
            ["b = a", "5", "抄 5 → 5", "（还不存在）"],
            ["a = 10", "5 → 10", "5（不动）", "（还不存在）"],
            ["c = a + b", "10", "5", "10+5 → 15"],
            ["print(a,b,c)", "10", "5", "15 → 输出 10 5 15"]
          ]
        },
        {
          'type': "h4",
          'text': "变式④：a = b = c —— 一条连等，给好几个变量一起赋值"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式④代码",
          'code': "a = b = c = 5   # a、b、c 三个盒子都放 5\nb = 9           # 只改 b → a=5, b=9, c=5\nc = b           # 把 b 现在的 9 抄给 c → c=9\nprint(a, b, c)  # 输出 5 9 9"
        },
        {
          'type': "p",
          'text': "<strong>变式④要点：</strong>一条 <code>a = b = c = 5</code> 等价于依次 <code>c=5; b=5; a=5</code>，三个盒子都得到 5。<strong>这之后它们是三个独立盒子</strong>：改 b 不影响 a 和 c；<code>c = b</code> 只是又把 b 当前的值抄了一份给 c。"
        },
        {
          'type': "table",
          'head': ["代码行", "a 盒子", "b 盒子", "c 盒子"],
          'rows': [
            ...
            ["b = 9", "5", "5 → 9", "5（不动）"],
            ["c = b", "5", "9", "抄 9 → 9"],
            ["print(a,b,c)", "5", "9", "9 → 输出 5 9 9"]
          ]
        },
        {
          'type': "h4",
          'text': "变式⑤：交换陷阱 a = b; b = a —— 没有第三个人接手会把值弄丢"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式⑤代码（错误示范）",
          'code': "x = 1\ny = 2\nx = y        # 把 y 的 2 抄给 x → x=2（原来的 1 被盖掉，丢了！）\ny = x        # 再把 x 的 2 抄给 y → y 也变 2\nprint(x, y)  # 输出 2 2 —— 1 没了，交换失败！"
        },
        {
          'type': "table",
          'head': ["代码行", "x 盒子", "y 盒子", "为什么"],
          'rows': [
            ["x = 1", "→ 1", "（还不存在）", ""],
            ["y = 2", "1", "→ 2", ""],
            ["x = y", "1 → 2", "2", "1 被 2 盖掉，丢了"],
            ["y = x", "2", "2 → 2", "x 已是 2，y 也成 2"],
            ["print(x,y)", "2", "2 → 输出 2 2", "交换失败"]
          ]
        },
        {
          'type': "p",
          'text': "<strong>变式⑤教训：</strong>想交换两个变量的值，必须备一个<strong>临时『第三只盒子』</strong>先把 x 的值存起来，否则一边的旧值会被覆盖丢失。<strong>好在 Python 提供了天生的一招——</strong>下面这个，请务必记住。"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式⑤的对：Python 一招交换 x, y = y, x",
          'code': "x = 1\ny = 2\nx, y = y, x   # 右边先一起读成 (2, 1)，再分别放回 x、y\nprint(x, y)  # 输出 2 1 —— 一次到位，谁都不丢"
        },
        {
          'type': "table",
          'head': ["代码行", "x 盒子", "y 盒子", "说明"],
          'rows': [
            ["x = 1", "→ 1", "（还不存在）", ""],
            ["y = 2", "1", "→ 2", ""],
            ["x, y = y, x", "1 → 2", "2 → 1", "右边先整体读出 (2,1)，再同时放回"],
            ["print(x,y)", "2", "1 → 输出 2 1", "两边都拿到对方的旧值"]
          ]
        },
        {
          'type': "h4",
          'text': "变式⑥：累加器（编程里最常用，total = total + 1 的思路）"
        },
        {
          'type': "code",
          'lang': "python",
          'title': "变式⑥代码：累计总分",
          'code': "total = 0\ntotal = total + 5   # 0 + 5 → 5\ntotal = total + 7   # 5 + 7 → 12\ntotal = total + 2   # 12 + 2 → 14\nprint(total)        # 输出 14"
        },
        {
          'type': "p",
          'text': "<strong>变式⑥要点：</strong>累加器是编程里出现频率最高的写法：每次把自己<strong>现在</strong>的值取出来，加上一个新数，再存回自己。它的节奏和变式②完全一样——<strong>先算右边，再盖左边</strong>，只是每次把累积的结果越滚越大。"
        },
        {
          'type': "table",
          'head': ["代码行", "total 旧值", "先算", "total 新值"],
          'rows': [
            ["total = 0", "—", "—", "0"],
            ["total = total + 5", "0", "0+5=5", "5"],
            ["total = total + 7", "5", "5+7=12", "12"],
            ["total = total + 2", "12", "12+2=14", "14"],
            ["print(total)", "14", "—", "打印 → 14"]
          ]
        },
        {
          'type': "tip",
          'title': "一眼看穿的万能技巧",
          'text': "再绕的多变量题，对每一个变量都只问一句话：<strong>它最后一次被赋值是在哪一行？赋的是多少？</strong>中间那些抄来抄去都会被后面的覆盖，不影响最终答案。再练一道：<code>p = 1; q = p; p = 2; r = p; print(q, p, r)</code> → 结果是 <code>1 2 2</code>（q 拿走第一个 1；p 最后是 2；r 从 p 拿到 2）。"
        },
        {
          'type': "info",
          'title': "等号是赋值不是相等",
        ...
        {
          'type': "keypoints",
          'items': [ "变量 = 给数据贴的标签，用 = 赋值",
            "Python 变量无需声明类型，自动推断",
            "命名用字母/数字/下划线，不能数字开头",
            "区分大小写，推荐 snake_case 命名",
            "= 是赋值，== 才是相等比较",
            "每个变量 = 一个盒子，一次只看一行、只盯一个变量",
            "y = x 只是把 x 此刻的值抄一份给 y，抄完即断",
            "v = v + 1：永远先算右边、再把结果盖进左边",
            "交换要留意，Python 用 x, y = y, x"
          ]
        }
      ],
      'templates': [
