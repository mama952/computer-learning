...
window.ERRORS_DATA = {
  ...
  icon: "错",
  description:
    "学编程一半的时间都在和报错搏斗。这里把最经典、最高频的报错、踩坑与误区做成『字典』：每个都讲清楚为什么错、怎么改、怎么预防。",
  children: [
    /* ================= Python 常见错误 ================= */
    {
      id: "err-python",
      ...
      lessons: [
        {
          ...
          difficulty: "入门",
          blocks: [
            { type: "p", text: "Python 用缩进代表代码块，所以缩进错误极常见；遇到中文没加声明、变量名拼写错了也会报一堆名。下面按住最典型的三种。" },
            { type: "code", lang: "bash", title: "三个高频报错原文", code: '# 报错1: IndentationError: expected an indented block\n# 原因: def/if/for 下面没有写缩进的代码块\ndef foo():\n    pass   # 之前这里空白会报错\n\n# 报错2: SyntaxError: Non-ASCII character\n# 原因: 代码含中文但没在文件开头声明编码(Python2)或在Python3用了错误\n# Python3 默认utf-8，一般不需声明；但控制台/文件需确保utf-8\n\n# 报错3: NameError: name \'xxx\' is not defined\n# 原因: 变量/函数名拼错，或在使用前未定义，或作用域外使用\nscores = 90\nprint(scroes)   # 拼错 NameError\n\n# 口诀: 报错给出行号, 先看行号再看名字' },
            { type: "table", head: ["报错", "含义", "对策"], rows: [["IndentationError", "缩进不一致/缺块", "统一 4 空格，检查混合 Tab"], ["SyntaxError", "语法错误", "看行号，查括号/引号/冒号"], ["NameError", "名字未定义", "查拼写、变量顺序、作用域"], ["TypeError", "类型不符", "如向 int 用 len()，检查类型"]] },
            { type: "keypoints", items: ["缩进即语法，统一 4 空格", "报错先看行号与变量名", "中文一般不会报错，UTF-8 即可", "用 print/调试确认变量真的存在"] },
          ],
        },
        {
          id: "err-py-2",
          ...
          difficulty: "进阶",
          blocks: [
            { type: "p", text: "这几个问题几乎是 Python 面试的必问陷阱，也是写代码最容易踩的隐性 bug。很多人代码『看着对』却出错，根源都在这里。" },
            { type: "code", lang: "python", title: "可变默认参数陷阱", code: '# 陷阱: 用可变对象(列表/字典)作默认参数\n# 默认参数在函数定义时只创建一次，会累积!  ← bug\ndef add(item, bucket=[]):\n    bucket.append(item)\n    return bucket\n\nprint(add("a"))   # [\'a\']\nprint(add("b"))   # [\'a\', \'b\']  ← 意外累积了!\n\n# 正确做法: 用 None 占位，函数内再创建\ndef add_ok(item, bucket=None):\n    if bucket is None:\n        bucket = []\n    bucket.append(item)\n    return bucket\n\nprint(add_ok("a"))   # [\'a\']\nprint(add_ok("b"))   # [\'b\']  ← 干净\n\n# 另一个坑: NoneType 没有某些属性\n# users = None; users.append(x)  → AttributeError: \'NoneType\'\n# 对策: 使用前判断 if users is not None:' },
            { type: "warn", title: "深浅拷贝", text: "直接 <code.inline>new_list = old_list</code.inline> 只是复制引用，改 rewritten 会影响原列表。要真正复制用 <code.inline>copy.copy()</code.inline>（浅）或 <code.inline>copy.deepcopy()</code.inline>（深）。" },
            { type: "keypoints", items: ["可变默认参数会在调用间累积——用 None 占位", "list 赋值=共享引用，改动互相影响", "None 参与运算会抛 AttributeError", "深拷贝用 import copy; copy.deepcopy()"] },
          ],
        },
      ],
    },
      ...
      lessons: [
        {
          ...
          difficulty: "进阶",
          blocks: [
            { type: "code", lang: "bash", title: "Git 报错与解法速查", code: '# 1. 推送被拒: \'non-fast-forward\'\n# 原因: 远程已有别人/自己的新提交, 而你本地落后\ngit pull --rebase        # 先变基拉取(推荐)\n# 或 git pull            # 直接合并\n# 再推送\ngit push\n\n# 2. 合并冲突 merge conflict\n# 会出现 <<<<<<< ======= >>>>>>> 标记\n# 手动编辑成最终想要的样子后:\ngit add 文件\ngit commit\n\n# 3. 误删/改坏, 还没提交 — 从暂存区或HEAD恢复\ngit checkout -- 文件      # 放弃工作区修改, 恢复成最新提交\n\n# 4. 提交回退 reset(慎用)\ngit log --oneline         # 先看哈希\ngit reset --hard <哈希>   # 回到该提交(会丢后面提交, 谨慎)\n\n# 5. 找回误删的提交(还没被GC)\ngit reflog                # 所有历史记录\ngit reset --hard <哈希>   # 从那恢复\n\n# 6. 撤销最新一次提交但保留改动\ngit reset --soft HEAD~1\n\n# 7. 忘记加文件, 补进上一个提交\ngit add 漏掉的文件\ngit commit --amend' },
            { type: "warn", title: "铁律", text: "<code.inline>reset --hard</code.inline> 会丢弃未提交的改动。动手前先 <code.inline>git status</code> 和 <code.inline>git stash</code> 存档（<code.inline>git stash</code> 可临时藏起改动）。" },
            { type: "keypoints", items: ["推送被拒先 pull --rebase 再 push", "冲突手动解决后 add+commit", "git reflog 能找回丢失的提交", "危险命令前先 stash 或备份"] },
          ],
        },
      ],
    },
      ...
      lessons: [
        {
          ...
          difficulty: "入门",
          blocks: [
            { type: "p", text: "报错不可怕，可怕的是『看着眼晕就瞎改』。记住这套七步法，把它当成条件反射，绝大多数问题都能 20 分钟内解决。" },
            { type: "code", lang: "text", title: "七步排错法", code: '1. 完整读报错: 错误类型 + 文件名 + 行号 + 堆栈\n   报错第一行的最后，往往是关键线索\n\n2. 弄清楚"正确的行为"是什么\n   先定义期望，才知道差距在哪\n\n3. 稳定复现\n   能稳定复现 = 已掌握一半；把条件缩到最小\n\n4. 打印中间值 / 打断点\n   在每个可能出错处看实际值，对比期望\n\n5. 二分定位\n   从两端缩小范围，确定"最初出错的那一行"\n\n6. 修复 + 立即验证\n   一次改一处，改完就跑，别一次改十处\n\n7. 写回归测试/记笔记\n   防止同样的坑再踩一遍\n\n配套心态:\n- 报错信息是你的朋友，不是骂你\n- 90% 的问题网上有人踩过，搜索整段报错大有用\n- 改一处测一处，胜过盲改十处' },
            { type: "keypoints", items: ["先读清报错类型+行号+堆栈再动手", "能稳定复现等于解决一半", "二分定位最初出错行", "一次改一处并及时验证", "学会搜索完整报错，站在前人肩膀上"] },
          ],
        },
      ],
    },
